...
describe('Tasklist', () => {
    ...
    describe('addTask', () => {
        it('devrait ajouter une tâche et lui assigner un ID', () => {
            const task = { title: 'Nouvelle tâche', status: 'pending' as const };
        ...
        it('devrait rejeter une tâche avec titre vide', () => {
            const task = { title: '   ', status: 'pending' as const };
            
            expect(() => tasklist.addTask(task)).toThrow('La tache ne peut pas etre vide');
        });

        it('devrait auto-incrémenter les IDs', () => {
            tasklist.addTask({ title: 'Tâche 1', status: 'pending' as const });
            tasklist.addTask({ title: 'Tâche 2', status: 'progress' as const });
    ...
    describe('removeTask', () => {
        it('devrait supprimer une tâche par ID', () => {
            tasklist.addTask({ title: 'À supprimer', status: 'pending' as const });
            const tasks = tasklist.listTasks();
            const id = tasks[0]?.id!;
    ...
    describe('updateTask', () => {
        it('devrait mettre à jour une tâche existante', () => {
            tasklist.addTask({ title: 'Original', status: 'pending' as const });
            const tasks = tasklist.listTasks();
            const id = tasks[0]?.id!;
        ...
        it('devrait rejeter un titre vide', () => {
            tasklist.addTask({ title: 'Test', status: 'pending' as const });
    ...
    describe('listTasks', () => {
        beforeEach(() => {
            tasklist.addTask({ title: 'Tâche pending', status: 'pending' as const });
            tasklist.addTask({ title: 'Tâche progress', status: 'progress' as const });
            tasklist.addTask({ title: 'Tâche finished', status: 'finished' as const });
        });
    ...
    describe('saveTasklist et loadTasklist', () => {
        it('devrait sauvegarder et recharger les tâches', () => {
            tasklist.addTask({ title: 'Persistante', status: 'pending' as const });
