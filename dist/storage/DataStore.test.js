...
describe('DataStore', () => {
    ...
    beforeEach(() => {
        ...
        store = new DataStore(testFilename);
    });
    afterEach(() => {
        if (fs.existsSync(testFilePath)) {
            fs.unlinkSync(testFilePath);
    ...
    describe('create', () => {
        ...
        it('devrait incrémenter les IDs correctement', () => {
            const item1 = { id: 0, name: 'Item 1' };
            const item2 = { id: 0, name: 'Item 2' };
            store.create(item1);
            store.create(item2);
    ...
    describe('findById', () => {
        it('devrait trouver un item par son ID', () => {
            const item = { id: 0, name: 'Trouvable' };
            store.create(item);
    ...
    describe('update', () => {
        it('devrait mettre à jour un item existant', () => {
            const item = { id: 0, name: 'Original' };
            store.create(item);
            const updated = { id: item.id, name: 'Modifié' };
        ...
        it('ne devrait rien faire si item non trouvé', () => {
            const item1 = { id: 0, name: 'Item 1' };
            store.create(item1);
            const fakeItem = { id: 999, name: 'Fake' };
            const updated = { id: 999, name: 'Updated' };
    ...
    describe('delete', () => {
        it('devrait supprimer un item existant', () => {
            const item = { id: 0, name: 'À supprimer' };
            store.create(item);
            store.delete(item);
            const items = store.readAll();
    ...
    describe('save et load', () => {
        ...
        it('devrait charger les données depuis un fichier', () => {
            store.create({ id: 0, name: 'Item 1' });
            store.save();
            const newStore = new DataStore(testFilename);
        ...
        it('ne devrait pas crasher si le fichier n\'existe pas', () => {
            expect(() => {
                const newStore = new DataStore('inexistant.json');
                newStore.load();
            }).not.toThrow();
        });
    });
